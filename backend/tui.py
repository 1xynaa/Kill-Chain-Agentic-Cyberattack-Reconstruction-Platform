...
async def run(paths: list[str], max_calls: int | None = None) -> int:
    ...
    workspace = settings.workspace_root / str(investigation.id)
    ...
    async def sink(event):
        ...
        elif event.type in {"finding", "stage_update", "provider_error", "status", "skill_select"}:
            print(f"\033[35m{event.type.upper()}\033[0m {json.dumps(payload, default=str)}")

    await Agent(settings, runner, Path(__file__).resolve().parents[1] / "skills").investigate(investigation, sink)
    print("\nREPORT")
