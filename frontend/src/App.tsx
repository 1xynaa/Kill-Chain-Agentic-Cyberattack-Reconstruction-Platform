...
function PrintReport({ report }: { report: any }) {
  ...
  const findings = report.timeline || []
  const iocs = report.iocs || []
  const stages = Object.entries(report.stages || {}) as [string, number][]
  const sectionText = (heading: string, nextHeading?: string) => {
    const text = report.narrative || ''
    const start = text.indexOf(`${heading}\n`)
    if (start < 0) return ''
    const contentStart = start + heading.length + 1
    const end = nextHeading ? text.indexOf(`\n\n${nextHeading}`, contentStart) : text.length
    return text.slice(contentStart, end < 0 ? text.length : end).trim()
  }
  const evidence = sectionText('EVIDENCE EXAMINED', 'ANALYST REASONING AND FINDINGS') || 'No evidence details available.'
  ...
  return (
    <main className="print-report">
      <header className="print-report-header">
        <div>
          <div className="print-eyebrow">KILL CHAIN // FORENSIC RECONSTRUCTION</div>
          <h1>Investigation Report</h1>
          <p className="print-report-id">{report.investigation_id}</p>
        </div>
        <div className="print-status">{String(report.status || 'unknown').toUpperCase()}</div>
      </header>

      <section className="print-summary-grid">
        <div><span>Findings</span><strong>{findings.length}</strong></div>
        <div><span>Indicators</span><strong>{iocs.length}</strong></div>
        <div><span>Confirmed stages</span><strong>{stages.length}/7</strong></div>
      </section>
      ...
      <section className="print-section">
        ...
        <div className="print-pre">{evidence}</div>
      </section>

      <section className="print-section">
        <h2>Kill-chain coverage</h2>
        <div className="print-stage-grid">
          {stages.map(([stage, score]) => <div key={stage}><span>{stage}</span><strong>{Math.round(score * 100)}%</strong></div>)}
        </div>
      </section>

      <section className="print-section">
        <h2>Analyst reasoning and findings</h2>
        {findings.length ? findings.map((finding: any, index: number) => (
          <article className="print-finding" key={finding.id || index}>
            <div className="print-finding-top"><span>#{String(index + 1).padStart(2, '0')} · {finding.stage || 'Unmapped'}</span><strong>{Math.round((finding.confidence || 0) * 100)}% confidence</strong></div>
            <h3>{finding.title}</h3>
            <p>{finding.description}</p>
            <div className="print-finding-meta"><span>Source: {finding.source_file || 'Unknown evidence'}</span><span>Time: {finding.timestamp || 'Unavailable'}</span></div>
            {!!finding.iocs?.length && <div className="print-finding-iocs">Indicators: {finding.iocs.join(' · ')}</div>}
            {finding.tentative && <div className="print-tentative">TENTATIVE / OUT OF SEQUENCE</div>}
          </article>
        )) : <p>No findings were produced from the available evidence.</p>}
      </section>

      {!!iocs.length && <section className="print-section">
        <h2>Extracted indicators</h2>
        <table className="print-ioc-table"><thead><tr><th>Type</th><th>Value</th></tr></thead><tbody>{iocs.map((ioc: any, index: number) => <tr key={`${ioc.value}-${index}`}><td>{ioc.type}</td><td>{ioc.value}</td></tr>)}</tbody></table>
      </section>}

      <section className="print-section">
        <h2>Forensic activity</h2>
        <p>{activity}</p>
      </section>
      <section className="print-section print-limitations">
        <h2>Limitations</h2>
        <p>{limitations}</p>
      </section>
      <footer className="print-footer">Generated by Kill Chain · Evidence-first reconstruction platform</footer>
    </main>
  )
}
